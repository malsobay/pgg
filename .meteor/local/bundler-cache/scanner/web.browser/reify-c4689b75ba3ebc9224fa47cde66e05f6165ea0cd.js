module.export({useHotkeys:()=>useHotkeys});let __spreadArray;module.link("tslib",{__spreadArray(v){__spreadArray=v}},0);let React;module.link("react",{"*"(v){React=v}},1);let HOTKEYS_PROVIDER_NOT_FOUND;module.link("../../common/errors",{HOTKEYS_PROVIDER_NOT_FOUND(v){HOTKEYS_PROVIDER_NOT_FOUND=v}},2);let comboMatches,getKeyCombo,parseKeyCombo;module.link("../../components/hotkeys/hotkeyParser",{comboMatches(v){comboMatches=v},getKeyCombo(v){getKeyCombo=v},parseKeyCombo(v){parseKeyCombo=v}},3);let HotkeysContext;module.link("../../context",{HotkeysContext(v){HotkeysContext=v}},4);/*
...
function useHotkeys(keys, options) {
    ...
    var _c = React.useContext(HotkeysContext), state = _c[0], dispatch = _c[1];
    if (!state.hasProvider) {
        React.useEffect(function () { return console.warn(HOTKEYS_PROVIDER_NOT_FOUND); }, []);
    }
    // we can still bind the hotkeys if there is no HotkeysProvider, they just won't show up in the dialog
    React.useEffect(function () {
        var payload = __spreadArray(__spreadArray([], globalKeys.map(function (k) { return k.config; }), true), localKeys.map(function (k) { return k.config; }), true);
